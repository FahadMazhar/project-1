	...
	<div class="content">
		...
		<div class="row">
			<div class="col-md-12">
			  ...
			  ?>              
      <select class="selectpicker" data-show-subtext="true" data-width="100%" id="contactid" data-live-search="true">
      <?php
	  $company=0;
      ...
      while($row = $result->fetch_assoc()) { $first=$row["firstname"]; $last=$row["lastname"]; $userid=$row["userid"];
	    $name=$first." ".$last;                     $sqll = "SELECT  * FROM tblclients WHERE userid='$userid'";
	                                                $resultt = $conn->query($sqll);
                                                    if ($resultt->num_rows > 0) {while($roww = $resultt->fetch_assoc()) {
														$company=$roww["company"];
														}} else {echo "0 results";}
	  ?>                                
          <option data-subtext="<?php echo $company;?>"><?php echo $name;?></option>
<?php }
} else {
  echo "0 results";
}
